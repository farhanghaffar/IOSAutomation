...
public class DIYVestPage extends BaseClass {
	...
	@FindBy(xpath = "//XCUIElementTypeOther[@name='diyVestListItem']")
...
//		Waits.wait3s();
	...
	public ArrayList<Double> getDIYVestPercentage() {
	  ...
	  public String getDIYVestCurrentInvestmentAmount() {
			return getElementText(currentInvestmentAmount).replace("$", "").trim();
		}
	  ...
	  public String getDIYVestSellAmount() {
			return diyVestSellAmount.getAttribute("value").replace("$", "").trim();// getElementText(currentInvestmentAmount);
		}
